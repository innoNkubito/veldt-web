...
function parseOverview(note: IntroductoryNoteFull): TextImageSection {
  const raw = note.pageContent
...
export default function IntroNoteRichContentTab({ introductoryNote, onSaved }: Props) {
  ...
  async function handleSave() {
    await updateIntroductoryNote(introductoryNote.id, { pageContent: { sections: [section] } })
    setDirty(false)
    onSaved?.()
  }
  ...
  return (
    <div>
      <S.SectionCard>
        <S.SectionHeader>
          <S.SectionLabel>Introductory Notes</S.SectionLabel>
        </S.SectionHeader>
        <S.SectionBody>
          <S.FieldGroup>
          ...
          <S.FieldGroup>
            ...
            <S.PhotoUploadZone htmlFor='intronote-overview-upload'>
            ...
            <input id='intronote-overview-upload' type='file' accept='image/*' multiple style={{ display: 'none' }} ref={fileRef} onChange={handleUpload} />
          </S.FieldGroup>
