...
import { useActivityStore, type ActivityFull } from '@/stores/activityStore'
import { uploadFile } from '@/lib/upload'
import type { TextImageSection } from '../../../properties/[id]/tabs/pageContent.types'
import ActivityRichContentTab from './ActivityRichContentTab'
...
function OverviewView({ section }: { section: TextImageSection }) {
  return (
    <S.ContentSection id="section-overview">
      <S.ContentSectionTitle>Activity Overview</S.ContentSectionTitle>
    ...
    <S.PreviewOverlay onClick={(e) => e.target === e.currentTarget && onClose()}>
      <S.PreviewBar>
        <S.PreviewBarTitle>
          ...
          {activity.name} — as seen by client
        </S.PreviewBarTitle>
        <S.PreviewCloseBtn onClick={onClose}>✕</S.PreviewCloseBtn>
      </S.PreviewBar>
      <S.PreviewBody>
        <S.PreviewCover $url={activity.coverImageUrl ?? undefined}>
          <S.PreviewCoverContent>
            {activity.area && <S.PageViewCoverBrand>{activity.area.name}</S.PageViewCoverBrand>}
            <S.PreviewCoverTitle>{activity.name}</S.PreviewCoverTitle>
            {activity.country && <S.PreviewCoverMeta>{activity.country}</S.PreviewCoverMeta>}
          </S.PreviewCoverContent>
        </S.PreviewCover>
        <S.PreviewSections>
          {!overview ? (
...
export default function ActivityPageContentTab({ activity }: Props) {
  const overview = parseOverview(activity.pageContent)
  ...
  async function handleCoverUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      await updateActivity(activity.id, { coverImageUrl: url })
  ...
  return (
    <>
      <S.AreaPageViewLayout>
        {/* ── Col 1: Cover ─────────────── */}
        <S.PageViewCover $url={activity.coverImageUrl ?? undefined}>
          <S.CoverUploadBtn htmlFor="activity-cover-upload" title="Change cover photo">
            {coverUploading ? '…' : (
              <>
          ...
          <S.PageViewCoverContent>
            {activity.area && <S.PageViewCoverBrand>{activity.area.name}</S.PageViewCoverBrand>}
            <S.PageViewCoverTitle>{activity.name}</S.PageViewCoverTitle>
            {activity.country && <S.PageViewCoverMeta>{activity.country}</S.PageViewCoverMeta>}
          </S.PageViewCoverContent>
        </S.PageViewCover>
      ...
      {showPreview && (
        <PreviewModal activity={activity} overview={overview} onClose={() => setShowPreview(false)} />
      )}
    </>
  )
}
