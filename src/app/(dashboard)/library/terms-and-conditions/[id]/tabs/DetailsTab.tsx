...
export default function TermsDetailsTab({ terms }: Props) {
  ...
  async function handleSave() {
    const input: UpdateTermsInput = { name: form.name || undefined, tags: form.tags }
    await updateTerms(terms.id, input)
    setDirty(false)
  }
  ...
  return (
    <div>
      ...
      <S.Card>
        ...
        <S.TagsRow onClick={() => document.getElementById('terms-tag-input')?.focus()}>
          {form.tags.map((tag) => (
            <S.TagBadge key={tag}>
              {tag}
