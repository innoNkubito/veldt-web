...
export default function AboutUsListPage() {
  ...
  const { aboutUsList, loading, saving, fetchAboutUsList, createAboutUs } = useAboutUsStore()
  ...
  useEffect(() => {
    if (client) fetchAboutUsList()
  }, [client])

  const filtered = useMemo(() => {
    const q = search.toLowerCase()
    if (!q) return aboutUsList
    return aboutUsList.filter(
      (a) =>
  ...
  async function handleCreate() {
    ...
    const created = await createAboutUs(newName.trim())
    if (created) {
      setShowCreate(false)
      setNewName('')
      router.push(`/library/about-us/${created.id}`)
    }
  }
    ...
    <S.PageRoot>
      <S.Header>
        <S.TitleGroup>
          ...
          <S.PageSubtitle>
            {loading ? 'Loading…' : `${aboutUsList.length} page${aboutUsList.length === 1 ? '' : 's'}`}
          </S.PageSubtitle>
        </S.TitleGroup>
        <S.HeaderRight>
          <S.SearchInput
            placeholder="Search about us pages…"
            value={search}
      ...
      {!loading && filtered.length === 0 ? (
        <S.EmptyState>
          <div>{search ? 'No pages match your search.' : 'No about us pages yet.'}</div>
          {!search && (
        ...
        <S.Table>
          ...
          <S.Tbody>
            {filtered.map((item) => (
              <S.Tr key={item.id} onClick={() => router.push(`/library/about-us/${item.id}`)}>
                <S.Td>
      ...
      {showCreate && (
        <S.Overlay onClick={() => setShowCreate(false)}>
          <S.ModalCard onClick={(e) => e.stopPropagation()}>
            <S.ModalTitle>New About Us Page</S.ModalTitle>
            <S.ModalSubtitle>You can fill in the details after creating it.</S.ModalSubtitle>
            <S.ModalLabel htmlFor="new-aboutus-name">Page title</S.ModalLabel>
            <S.ModalInput
              id="new-aboutus-name"
              autoFocus
              value={newName}
